...
configuro:
    ...
    bsf TRISB, 0, 1 ;((PORTB) and 0FFh), 0, a como entrada
    bcf ANSELB, 0, 1 ;((PORTB) and 0FFh), 0, a como digital
    bcf TRISC, 0, 1 ;((PORTC) and 0FFh), 0, a como salida
    bcf ANSELC, 0, 1 ;((PORTC) and 0FFh), 0, a como digital
inicio: ;codigo de la aplicacion
    btfss PORTB, 0, 1 ;pregunto si ((PORTB) and 0FFh), 0, a es uno
    bra es_falso ;rpta falso, salta a label es_falso
    bcf LATC, 0, 1 ;rpta verdad, ((PORTC) and 0FFh), 0, a en cero
    bra inicio ;salto a label inicio
es_falso:
    bsf LATC, 0, 1 ;((PORTC) and 0FFh), 0, a en uno
    bra inicio ;salto a label inicio
    ;Fin del PSECT
    end upcino
